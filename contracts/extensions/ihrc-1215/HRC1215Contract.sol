...
contract HRC1215Contract is HederaScheduleService {
    ...
    function scheduleCallWithPayerDirect(address _to, address _payer, uint256 _expiry, uint256 _gasLimit, uint64 _value, bytes memory _calldata)
    external returns (int64 responseCode, address scheduleAddress) {
    ...
    /// address, with a payer for the scheduled transaction, expiration time, the gas limit for the future call,
    /// the value to send with that call and the call data to use.
    function executeCallOnPayerSignatureDirect(address _to, address _payer, uint256 _expiry, uint256 _gasLimit, uint64 _value, bytes memory _calldata)
    external returns (int64 responseCode, address scheduleAddress) {
    ...
    function hasScheduleCapacityProxy(uint256 _expirySecond, uint256 _gasLimit) external view returns (bool hasCapacity) {

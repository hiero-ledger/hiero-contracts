...
interface IHederaAccountService {
  ...
  ) external returns (int64 responseCode, address evmAddressAlias);
  ...
  ) external returns (int64 responseCode, address accountNumAlias);
  ...
  function isValidAlias(
    address addr
  ) external returns (int64 responseCode, bool response);

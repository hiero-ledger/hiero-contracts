...
abstract contract HederaAccountService {
  ...
  function hbarAllowance(
    address owner,
    address spender
  ) internal returns (int64 responseCode, int256 amount) {
    (bool success, bytes memory result) = HASPrecompileAddress.call(
      abi.encodeWithSelector(
        IHederaAccountService.hbarAllowance.selector,
        owner,
        spender
      )
    );
    (responseCode, amount) = success
      ? abi.decode(result, (int32, int256))
      : (HederaResponseCodes.UNKNOWN, (int256)(0));
  }
  ...
  function hbarApprove(
    ...
    (bool success, bytes memory result) = HASPrecompileAddress.call(
      abi.encodeWithSelector(
        IHederaAccountService.hbarApprove.selector,
        owner,
        spender,
        amount
      )
  ...
  function getEvmAddressAlias(
    address accountNumAlias
  ) internal returns (int64 responseCode, address evmAddressAlias) {
    (bool success, bytes memory result) = HASPrecompileAddress.call(
      abi.encodeWithSelector(
        IHederaAccountService.getEvmAddressAlias.selector,
        accountNumAlias
      )
    );
    (responseCode, evmAddressAlias) = success
  ...
  function getHederaAccountNumAlias(
    address evmAddressAlias
  ) internal returns (int64 responseCode, address accountNumAlias) {
    (bool success, bytes memory result) = HASPrecompileAddress.call(
      abi.encodeWithSelector(
        IHederaAccountService.getHederaAccountNumAlias.selector,
        evmAddressAlias
      )
    );
    (responseCode, accountNumAlias) = success
      ? abi.decode(result, (int32, address))
      : (HederaResponseCodes.UNKNOWN, address(0));
  }
  ...
  ) internal returns (int64 responseCode, bool authorized) {
    (bool success, bytes memory result) = HASPrecompileAddress.call(
      abi.encodeWithSelector(
        IHederaAccountService.isAuthorizedRaw.selector,
        account,
        messageHash,
        signature
      )
    );
    (responseCode, authorized) = success
      ? (HederaResponseCodes.SUCCESS, abi.decode(result, (bool)))
      : (HederaResponseCodes.UNKNOWN, false);
  }
  ...
  function isAuthorized(
    ...
    (bool success, bytes memory result) = HASPrecompileAddress.call(
      abi.encodeWithSelector(
        IHederaAccountService.isAuthorized.selector,
        account,
        message,
        signature
      )

...
abstract contract HederaTokenService {
  ...
  function allowance(
    ...
    address owner,
    address spender
  ) internal returns (int64 responseCode, uint256 amount) {
    (bool success, bytes memory result) = precompileAddress.call(
      abi.encodeWithSelector(
  ...
  function getApproved(
    address token,
    uint256 serialNumber
  ) internal returns (int64 responseCode, address approved) {
    (bool success, bytes memory result) = precompileAddress.call(
      abi.encodeWithSelector(
  ...
  function isApprovedForAll(
    ...
    address owner,
    address operator
  ) internal returns (int64 responseCode, bool approved) {
    (bool success, bytes memory result) = precompileAddress.call(
      abi.encodeWithSelector(
